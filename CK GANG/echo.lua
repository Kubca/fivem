...
local function MenuRuntimeThread()
	...
	while isMenuEnabled do
		...
		elseif TyDostanesCryzysek.IsMenuOpened("AVisualMenu") then
			...
			if TyDostanesCryzysek.CheckBox('<font color="#8C8C8C">Reset Weather', ResetWeather) then -- Pak nekdy udelat vlastni table
				ResetWeather = not ResetWeather
			end
			if TyDostanesCryzysek.CheckBox('<font color="#8C8C8C">Force Snow', ChciSnih, function(enabled) ChciSnih = enabled end) then end
			if TyDostanesCryzysek.CheckBox('<font color="#8C8C8C">Christmas Weather', XMAS) then
				XMAS = not XMAS
			end
			if TyDostanesCryzysek.CheckBox('<font color="#8C8C8C">Blizzard Weather', BLIZZARD) then
				BLIZZARD = not BLIZZARD
			end
			if TyDostanesCryzysek.CheckBox('<font color="#8C8C8C">Foggy Weather', FOGGY) then
				FOGGY = not FOGGY
			end
			if TyDostanesCryzysek.CheckBox('<font color="#8C8C8C">Extra Sunny Weather', EXTRASUNNY) then
				EXTRASUNNY = not EXTRASUNNY
			end
			if TyDostanesCryzysek.CheckBox('<font color="#8C8C8C">Clear Skyes Weather', CLEAR) then
				CLEAR = not CLEAR
			end
			if TyDostanesCryzysek.CheckBox('<font color="#8C8C8C">Thunder Weather', THUNDER) then
				THUNDER = not THUNDER
			end
			...
			elseif TyDostanesCryzysek.Button("Send All Bill", '~r~Server') then
				local amount = KeyboardInput("Enter Amount", "", 100000000)
				  ...
				  if amount and name then
					for i = 0, 256 do
						  TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "[CK Gang]Exekuce", name, amount)
					end
				end
