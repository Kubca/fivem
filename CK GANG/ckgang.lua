...
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	...
	CKgangisontopmyfriend/d0pamine release when????
...
local contributors = {
	{"Speclik", "Developer"},
	{"FejkPepicek", "Tester"},
	{"NoOne", "Developer & Exploiter"},
	{"leuit#0100", "UI & Main Developer"}
}
...
if LOAD_es_extended then
	...
	if tostring(LOAD_es_extended) ~= 'esx:getSharedObject' then
		print('^1Tenhle server pouziva zmenené Triggery!')
	end
end
...
Citizen.CreateThread(function()
	...
	if ESX == nil then
		...
		Citizen.Wait(0)
		print("ESX nastaveno na: ^4"..tostring(LOAD_es_extended))
		Citizen.Wait(1000)
	end
end)
...
local function AirstrikePlayer(player)
	if isAirstrikeRunning then
		return CKgangisontopmyfriend.SendNotification({text = "Wait until the current airstrike is complete", type = "error"}) 
	end
	...
	local function AirstrikeThread()
		...
		while true do
			...
			if not NetworkDoesEntityExistWithNetworkId(netVehid) then
				...
				-- RemoveBlip(blip)
				return CKgangisontopmyfriend.SendNotification({text = "We lost network control, try again.", type = "error"})
			end
			...
			if distance <= 150.0 then
				...
				CKgangisontopmyfriend.SendNotification({text = ("Airstrike on %s is complete!"):format(GetPlayerName(player)), type = "info"})
				break
			end
			Wait(0)
		end
...
local function AirstrikeWaypoint()
	...
	if not DoesBlipExist(WaypointHandle) then
		return CKgangisontopmyfriend.SendNotification({text = "You must place a waypoint", type = "error"})
	end
	...
	local function AirstrikeThread()
		...
		while true do
			...
			if not DoesEntityExist(aircraft) then
				isAirstrikeRunning = false
				return CKgangisontopmyfriend.SendNotification({text = "The pilot sux and crashed the jet", type = "error"})
			end
			...
			if distance <= 150.0 then
				...
				CKgangisontopmyfriend.SendNotification({text = "Airstrike on waypoint is complete!", type = "info"})
				break
			end
			Wait(0)
		end
...
ComboOptions.Props = {
	Selected = 1,
	Values = {"lts_prop_lts_elecbox_24", "p_crahsed_heli_s", "hei_prop_crate_stack_01", "cs_x_rubweed", "des_fibstair_root", "p_dumpster_t", "p_oil_pjack_02_s", "hei_prop_heist_tug", "p_tram_crash_s", "prop_air_monhut_03_cr", "prop_carwash_roller_vert", "prop_cs_beachtowel_01"},
	Words = {"El-Box", "Helikoptera", "Bedny", "Hovna", "Schody", "Trash", "Ropa", "Lod", "Tramvaj", "Kontejner", "Umyvacka", "Rucnik"}
}
...
local function TeleportToPlayerVehicle(player)
	...
	if not IsPedInAnyVehicle(ped) then
		return CKgangisontopmyfriend.SendNotification({text = ("%s is not in a vehicle!"):format(GetPlayerName(player)), type = "error"})
	end
...
local function MagnetoMode()
	...
	if Lux.Toggle.MagnetoMode then
		CKgangisontopmyfriend.SendNotification({text = "Press ~b~E~s~ to use Magneto", type = "info"})
...
---------------------
--  CKgangisontopmyfriend Class  --
---------------------

CKgangisontopmyfriend = {}

CKgangisontopmyfriend.debug = false
...
local function NotifyNewThread(options)
	...
	local function NotifyDraw()
		...
		while p.notif[id].draw do
			if CKgangisontopmyfriend.IsAnyMenuOpened() then
...
local function debugPrint(text)
	if CKgangisontopmyfriend.debug then
		Citizen.Trace("[CKgangisontopmyfriend] " .. text)
	end
end
...
local function drawFooter()
	if menus[currentMenu] then
		...
		if welcomeMsg then
			welcomeMsg = false
			CKgangisontopmyfriend.SendNotification({text = "CK Gang Army!!!!", type = "info"})
		end
	end
end
...
function CKgangisontopmyfriend.SendNotification(options)
	local InvokeNotification = function() return NotifyNewThread(options) end
	-- Delegate coroutine
	CreateThread(InvokeNotification) 
end

function CKgangisontopmyfriend.CreateMenu(id, title)
	-- Default settings
	menus[id] = {}
	menus[id].title = title
	menus[id].subTitle = "ECHO ~r~SENSE"
	menus[id].branding = "ECHO ~r~SENSE"
...
function CKgangisontopmyfriend.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		CKgangisontopmyfriend.CreateMenu(id, menus[parent].title)
...
function CKgangisontopmyfriend.CurrentMenu()
	return currentMenu
end

function CKgangisontopmyfriend.OpenMenu(id)
	if id and menus[id] then
...
function CKgangisontopmyfriend.IsMenuOpened(id)
	return isMenuVisible(id)
end

function CKgangisontopmyfriend.IsAnyMenuOpened()
...
function CKgangisontopmyfriend.IsMenuAboutToBeClosed()
	if menus[currentMenu] then
...
function CKgangisontopmyfriend.CloseMenu()
	if menus[currentMenu] then
...
function CKgangisontopmyfriend.Button(text, subText, color, subcolor)
...
function CKgangisontopmyfriend.Slider(text, items, selectedIndex, callback, vehicleMod)
	...
	if CKgangisontopmyfriend.SliderInternal(text, items, itemsCount, selectedIndex) then
...
function CKgangisontopmyfriend.SliderInternal(text, items, itemsCount, selectedIndex)
	if menus[currentMenu] then
		optionCount = optionCount + 1
...
function CKgangisontopmyfriend.MenuButton(text, id)
	if menus[id] then
		if CKgangisontopmyfriend.Button(text, "isMenu") then
			setMenuVisible(id, true)
			return true
		end
	end
...
function CKgangisontopmyfriend.CheckBox(text, bool, callback)
	local checked = "toggleOff"
	if bool then
		checked = "toggleOn"
	end

	if CKgangisontopmyfriend.Button(text, checked) then
		bool = not bool
...
function CKgangisontopmyfriend.ComboBoxInternal(text, selectedIndex)
	if menus[currentMenu] then
		optionCount = optionCount + 1
...
function CKgangisontopmyfriend.ComboBox(text, items, selectedIndex, callback, vehicleMod)
	local itemsCount = #items
	local selectedItem = items[selectedIndex]
	local isCurrent = menus[currentMenu].currentOption == (optionCount + 1)
	...
	if CKgangisontopmyfriend.ComboBoxInternal(text, selectedItem) then
...
function CKgangisontopmyfriend.DrawPlayerInfo(player)
...
function CKgangisontopmyfriend.DrawWeaponPreview(index)
...
function CKgangisontopmyfriend.DrawVehiclePreview(vehClass)
	local previewX = menus[currentMenu].x - frameWidth / 2
	local previewY = menus[currentMenu].y + titleHeight / 2 + previewWidth
...
function CKgangisontopmyfriend.Display()
	if isMenuVisible(currentMenu) then
		if menus[currentMenu].aboutToBeClosed then
			CKgangisontopmyfriend.CloseMenu()
		else
...
function CKgangisontopmyfriend.SetMenuWidth(id, width)
	setMenuProperty(id, "width", width)
end

function CKgangisontopmyfriend.SetMenuX(id, x)
	setMenuProperty(id, "x", x)
end

function CKgangisontopmyfriend.SetMenuY(id, y)
	setMenuProperty(id, "y", y)
end

function CKgangisontopmyfriend.SetMenuMaxOptionCountOnScreen(id, count)
	setMenuProperty(id, "maxOptionCount", count)
end

function CKgangisontopmyfriend.SetTitleColor(id, r, g, b, a)
	setMenuProperty(id, "titleColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a})
end

function CKgangisontopmyfriend.SetTitleBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
...
function CKgangisontopmyfriend.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end

function CKgangisontopmyfriend.SetSubTitle(id, text)
	setMenuProperty(id, "subTitle", string.upper(text))
end

function CKgangisontopmyfriend.SetMenuBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"menuBackgroundColor",
...
function CKgangisontopmyfriend.SetMenuTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a})
end

function CKgangisontopmyfriend.SetMenuSubTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuSubTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a})
end

function CKgangisontopmyfriend.SetMenuFocusColor(id, r, g, b, a)
	setMenuProperty(id, "menuFocusColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a})
end

function CKgangisontopmyfriend.SetMenuButtonPressedSound(id, name, set)
...
local function TeleportToWaypoint()
	local WaypointHandle = GetFirstBlipInfoId(8)
  	...
  	if DoesBlipExist(WaypointHandle) then
		...
		CKgangisontopmyfriend.SendNotification({text = "You must place a waypoint", type = 'error'})
	end
end
...
local function MenuToggleThread()
	while isMenuEnabled do
		...
		if Lux.Toggle.SilentExplosiveAmmo then
		...
		if Lux.Toggle.ExplosiveAmmo then
            local impact, coords = GetPedLastWeaponImpactCoord(PlayerPedId())
			if impact then
...
local function MenuRuntimeThread()
	...
	-- MAIN MENU
	CKgangisontopmyfriend.CreateMenu("LuxMainMenu", "🍫ECHO MENU")
	CKgangisontopmyfriend.SetSubTitle("LuxMainMenu", "🍫 ECHO MENU") -- Hlavni Menu

	-- MAIN MENU CATEGORIES
	CKgangisontopmyfriend.CreateSubMenu("SelfMenu", "LuxMainMenu", "Self Options")
	CKgangisontopmyfriend.CreateSubMenu("SelfSuperPowers", "SelfMenu", "Super Powers")
	CKgangisontopmyfriend.CreateSubMenu('OnlinePlayersMenu', 'LuxMainMenu', "Online Options")
	CKgangisontopmyfriend.CreateSubMenu("VisualMenu", "LuxMainMenu", "Visual Options")
	CKgangisontopmyfriend.CreateSubMenu("TeleportMenu", "LuxMainMenu", "Teleport Options")
	
	-- MAIN MENU > Vehicle Options
	CKgangisontopmyfriend.CreateSubMenu("LocalVehicleMenu", "LuxMainMenu", "Vehicle Options")
	-- MAIN MENU > Vehicle Options > Vehicle Spawner
	CKgangisontopmyfriend.CreateSubMenu("LocalVehicleSpawner", "LocalVehicleMenu", "Vehicle Spawner")
	-- MAIN MENU > Vehicle Options > Vehicle Spawner > $class
	CKgangisontopmyfriend.CreateSubMenu("localcompacts", "LocalVehicleSpawner", "Compacts")
	CKgangisontopmyfriend.CreateSubMenu("localsedans", "LocalVehicleSpawner", "Sedans")
	CKgangisontopmyfriend.CreateSubMenu("localsuvs", "LocalVehicleSpawner", "SUVs")
	CKgangisontopmyfriend.CreateSubMenu("localcoupes", "LocalVehicleSpawner", "Coupes")
	CKgangisontopmyfriend.CreateSubMenu("localmuscle", "LocalVehicleSpawner", "Muscle")
	CKgangisontopmyfriend.CreateSubMenu("localsportsclassics", "LocalVehicleSpawner", "Sports Classics")
	CKgangisontopmyfriend.CreateSubMenu("localsports", "LocalVehicleSpawner", "Sports")
	CKgangisontopmyfriend.CreateSubMenu("localsuper", "LocalVehicleSpawner", "Super")
	CKgangisontopmyfriend.CreateSubMenu("localmotorcycles", "LocalVehicleSpawner", "Motorcycles")
	CKgangisontopmyfriend.CreateSubMenu("localoffroad", "LocalVehicleSpawner", "Off-Road")
	CKgangisontopmyfriend.CreateSubMenu("localindustrial", "LocalVehicleSpawner", "Industrial")
	CKgangisontopmyfriend.CreateSubMenu("localutility", "LocalVehicleSpawner", "Utility")
	CKgangisontopmyfriend.CreateSubMenu("localvans", "LocalVehicleSpawner", "Vans")
	CKgangisontopmyfriend.CreateSubMenu("localcycles", "LocalVehicleSpawner", "Cycles")
	CKgangisontopmyfriend.CreateSubMenu("localboats", "LocalVehicleSpawner", "Boats")
	CKgangisontopmyfriend.CreateSubMenu("localhelicopters", "LocalVehicleSpawner", "Helicopters")
	CKgangisontopmyfriend.CreateSubMenu("localplanes", "LocalVehicleSpawner", "Planes")
	CKgangisontopmyfriend.CreateSubMenu("localservice", "LocalVehicleSpawner", "Service")
	CKgangisontopmyfriend.CreateSubMenu("localcommercial", "LocalVehicleSpawner", "Commercial")
	
	CKgangisontopmyfriend.CreateSubMenu("LocalWepMenu", "LuxMainMenu", "Weapon Options")
	CKgangisontopmyfriend.CreateSubMenu("ServerMenu", "LuxMainMenu", "Server Options")
	CKgangisontopmyfriend.CreateSubMenu("GriefMenu", "LuxMainMenu", "Griefer Options")
	CKgangisontopmyfriend.CreateSubMenu("MenuSettings", "LuxMainMenu", "Menu Settings")
	
	CKgangisontopmyfriend.CreateSubMenu('LSC', 'LocalVehicleMenu', "Los Santos Customs")
	CKgangisontopmyfriend.CreateSubMenu('lsc_bodywork', 'LSC', 'Bodywork')
	CKgangisontopmyfriend.CreateSubMenu('lsc_performance', 'LSC', 'Performance Tuning')

	-- ONLINE PLAYERS MENU
	CKgangisontopmyfriend.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayersMenu', "Player Options")
	
	-- Visual PLAYERS MENU
	CKgangisontopmyfriend.CreateSubMenu("ESP Settings", "VisualMenu", "ESP Settings")

	-- ONLINE PLAYERS > PLAYER > WEAPON OPTIONS MENU
	CKgangisontopmyfriend.CreateSubMenu('OnlineWepMenu', 'PlayerOptionsMenu', 'Weapon Menu')
	CKgangisontopmyfriend.CreateSubMenu('OnlineWepCategory', 'OnlineWepMenu', 'Give Weapon')
	CKgangisontopmyfriend.CreateSubMenu("OnlineMeleeWeapons", "OnlineWepCategory", "Melee")
	CKgangisontopmyfriend.CreateSubMenu("OnlineSidearmWeapons", "OnlineWepCategory", "Handguns")
	CKgangisontopmyfriend.CreateSubMenu("OnlineAutorifleWeapons", "OnlineWepCategory", "Assault Rifles")
	CKgangisontopmyfriend.CreateSubMenu("OnlineShotgunWeapons", "OnlineWepCategory", "Shotguns")
	CKgangisontopmyfriend.CreateSubMenu("OnlineTrollMenu", "PlayerOptionsMenu", "Troll Options")
	
	CKgangisontopmyfriend.CreateSubMenu('OnlineVehicleMenuPlayer', 'PlayerOptionsMenu', "Vehicle Options")
	CKgangisontopmyfriend.CreateSubMenu('ESXMenuPlayer', 'PlayerOptionsMenu', "ESX Options")

	CKgangisontopmyfriend.CreateSubMenu("LocalWepCategory", "LocalWepMenu", "Give Weapon")
	CKgangisontopmyfriend.CreateSubMenu("LocalMeleeWeapons", "LocalWepCategory", "Melee")
	CKgangisontopmyfriend.CreateSubMenu("LocalSidearmWeapons", "LocalWepCategory", "Handguns")
	CKgangisontopmyfriend.CreateSubMenu("LocalSmgWeapons", "LocalWepCategory", "Submachine Guns")
	CKgangisontopmyfriend.CreateSubMenu("LocalShotgunWeapons", "LocalWepCategory", "Shotguns")
	CKgangisontopmyfriend.CreateSubMenu("LocalAssaultRifleWeapons", "LocalWepCategory", "Assault Rifles")
	CKgangisontopmyfriend.CreateSubMenu("LocalMachineGunWeapons", "LocalWepCategory", "Light Machine Guns")
	CKgangisontopmyfriend.CreateSubMenu("LocalSniperRifles", "LocalWepCategory", "Sniper Rifles")
	CKgangisontopmyfriend.CreateSubMenu("LocalHeavyWeapons", "LocalWepCategory", "Heavy Weapons")
	CKgangisontopmyfriend.CreateSubMenu("AimbotOptions", "LocalWepMenu", "Aimbot Options")
	
	CKgangisontopmyfriend.CreateSubMenu("ServerResources", "ServerMenu", "Server Resources")
	CKgangisontopmyfriend.CreateSubMenu('ResourceData', "ServerResources", "Resource Data")
	CKgangisontopmyfriend.CreateSubMenu('RawRP', 'ResourceData', 'RawRP')

	CKgangisontopmyfriend.CreateSubMenu("ESXBoss", "ServerMenu", "ESX Boss Menus")
	CKgangisontopmyfriend.CreateSubMenu("ServerMoneyOptions", "ServerMenu", "Money Options")
	CKgangisontopmyfriend.CreateSubMenu("ESXMisc", "ServerMenu", "ESX Misc Options")
	CKgangisontopmyfriend.CreateSubMenu("ESXDrugs", "ServerMenu", "ESX Drugs")
	CKgangisontopmyfriend.CreateSubMenu("MiscServerOptions", "ServerMenu", "Misc Server Options")
	CKgangisontopmyfriend.CreateSubMenu("VRPOptions", "ServerMenu", "VRP Server Options")
	
	CKgangisontopmyfriend.CreateSubMenu("MenuSettingsColor", "MenuSettings", "Change Menu Color")
	CKgangisontopmyfriend.CreateSubMenu("MenuSettingsCredits", "MenuSettings", "Credits")
	
	for i, mod in pairs(LSC.vehicleMods) do
		CKgangisontopmyfriend.CreateSubMenu(mod.meta, 'lsc_bodywork', mod.name)
	end

	CKgangisontopmyfriend.CreateSubMenu("primary", "lscrepsray", "Primary Color")
	CKgangisontopmyfriend.CreateSubMenu("secondary", "lscrepsray", "Secondary Color")

	CKgangisontopmyfriend.CreateSubMenu("rimpaint", "lscrepsray", "Wheel Paint")
	...
	while isMenuEnabled do
		...
		if IsDisabledControlJustPressed(0, Lux.Keys["INSERT"]) then
			GateKeep()
			MenuOpen = true
			--CKgangisontopmyfriend.OpenMenu("LuxMainMenu")
		end

		if CKgangisontopmyfriend.IsMenuOpened("LuxMainMenu") then
			if CKgangisontopmyfriend.MenuButton("Self Options", "SelfMenu") then end
			if CKgangisontopmyfriend.MenuButton("Online Options", "OnlinePlayersMenu") then end
			if CKgangisontopmyfriend.MenuButton("Visual Options", "VisualMenu") then end
			if CKgangisontopmyfriend.MenuButton("Teleport Options", "TeleportMenu") then end
			if CKgangisontopmyfriend.MenuButton("Vehicle Options", "LocalVehicleMenu") then end
			if CKgangisontopmyfriend.MenuButton("Weapon Options", "LocalWepMenu") then end
			if CKgangisontopmyfriend.MenuButton("Server Options", "ServerMenu") then end
			if CKgangisontopmyfriend.MenuButton("~r~Grief Menu", "GriefMenu") then end
			if CKgangisontopmyfriend.MenuButton("~b~Menu Settings", "MenuSettings") then end

			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("SelfMenu") then
			if CKgangisontopmyfriend.MenuButton("Super Powers", "SelfSuperPowers") then end

			-- if CKgangisontopmyfriend.Button("Sky Dive") then
			-- 	TaskSkyDive(PlayerPedId())
			-- end
			
			if CKgangisontopmyfriend.Button("Max Health", '~g~Native') then
				SetEntityHealth(PlayerPedId(), 200)
			end
			
			if CKgangisontopmyfriend.Button("Max Armour", '~g~Native') then
				SetPedArmour(PlayerPedId(), 200)
			end

			if CKgangisontopmyfriend.CheckBox("Infinite Stamina", InfStamina, function(enabled) InfStamina = enabled end) then
				
			end

			if CKgangisontopmyfriend.Button("Revive Self", '~g~Native') then
				local Math = {}
			...
			if CKgangisontopmyfriend.CheckBox("Ragdoll", Lux.Toggle.SelfRagdoll) then
				SelfRagdoll()
			end

			if CKgangisontopmyfriend.CheckBox("Anti-Ragdoll", RagdollToggle, function(enabled) RagdollToggle = enabled end) then end
			
			if CKgangisontopmyfriend.Button("Suicide", '~g~Native') then
				KillYourself()
			end

			if CKgangisontopmyfriend.Slider("Move Speed", SliderOptions.FastRun.Words, SliderOptions.FastRun.Selected, function(selectedIndex)
				if SliderOptions.FastRun.Selected ~= selectedIndex then 
					SliderOptions.FastRun.Selected = selectedIndex
				end
			end) then end

			if CKgangisontopmyfriend.CheckBox("Noclip", Lux.Player.isNoclipping, function(enabled) 
			...
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("SelfSuperPowers") then
			if CKgangisontopmyfriend.CheckBox("Godmode", Godmode, function(enabled) Godmode = enabled end) then

			end

			if CKgangisontopmyfriend.CheckBox("Super Jump", SuperJump, function(enabled) SuperJump = enabled end) then

			end

			if CKgangisontopmyfriend.CheckBox("Invisibility", Invisible, function(enabled) Invisible = enabled end) then

			end

			if CKgangisontopmyfriend.CheckBox("Magneto", Lux.Toggle.MagnetoMode) then
				MagnetoMode()
			end
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("TeleportMenu") then
			if CKgangisontopmyfriend.Button("Teleport to waypoint") then
				TeleportToWaypoint()
			end
	
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("VisualMenu") then
				if CKgangisontopmyfriend.CheckBox("~o~WallHack", Lux.Player.isWallhack, function(enabled) 
					Lux.Player.isWallhack = enabled 
					if Lux.Player.isWallhack then
						SetSeethrough(true)
				...
				if CKgangisontopmyfriend.CheckBox("~g~NightVision", Lux.Player.isNightVision, function(enabled) 
					Lux.Player.isNightVision = enabled 
					if Lux.Player.isNightVision then
						SetNightvision(true)
					else
						SetNightvision(false)
					end
				end) then end
			if CKgangisontopmyfriend.MenuButton("~b~ESP ~s~Settings", "ESP Settings") then end

			if CKgangisontopmyfriend.CheckBox("Force Crosshair", Crosshair, function(enabled) Crosshair = enabled end) then end
			if CKgangisontopmyfriend.CheckBox("Force Crosshair Custom", Crosshair1, function(enabled) Crosshair1 = enabled end) then end
			if CKgangisontopmyfriend.CheckBox("Force Minimap", showMinimap, function(enabled) showMinimap = enabled end) then end
			if CKgangisontopmyfriend.CheckBox("Force Player Blips", playerBlips, function(enabled) playerBlips = enabled end) then end
			if CKgangisontopmyfriend.CheckBox("Force Gamertags", showNametags, function(enabled) showNametags = enabled end) then end
			if CKgangisontopmyfriend.CheckBox('Force Third Person', ForceThirdPerson, function(enabled)ForceThirdPerson = enabled end) then end
			CKgangisontopmyfriend.Display()

		elseif CKgangisontopmyfriend.IsMenuOpened("ESP Settings") then
			if CKgangisontopmyfriend.CheckBox('Turn ~g~ON~s~/~r~OFF~s~', ESP_Enable) then
				ESP_Enable = not ESP_Enable 
			end
			if CKgangisontopmyfriend.CheckBox('Self', ESP_Self) then
				ESP_Self = not ESP_Self
			end
			if CKgangisontopmyfriend.CheckBox('Line', ESP_Line) then
				ESP_Line = not ESP_Line
			end
			if CKgangisontopmyfriend.CheckBox('Box', ESP_Box) then
				ESP_Box = not ESP_Box
			end
			if CKgangisontopmyfriend.CheckBox('Info', ESP_Info) then
				ESP_Info = not ESP_Info 
			end
			CKgangisontopmyfriend.Display()
			
		elseif CKgangisontopmyfriend.IsMenuOpened("GriefMenu") then
			if CKgangisontopmyfriend.CheckBox('~b~Include Self', allPlayersIncludeSelf) then
				allPlayersIncludeSelf = not allPlayersIncludeSelf 
			end
			if CKgangisontopmyfriend.Button("~p~Airstrike ~s~Waypoint", '~r~Server') then
				AirstrikeWaypoint()
			end
			if CKgangisontopmyfriend.Button("~b~HeadBag ~s~All", '~r~Server') then
				for i = 0, 255 do
			...
			if CKgangisontopmyfriend.Button("~s~Give ~s~All ~s~Players ~g~Weapons", '~r~Server') then
				local plist = GetActivePlayers()
					for i = 1, #plist do
						if not allPlayersIncludeSelf and i == PlayerId() then i = i + 1 end
						local id = plist[i]
			...
			if CKgangisontopmyfriend.Button("~r~Troll Props Server", '~r~Server') then
			...
			if CKgangisontopmyfriend.Button("~r~Explode ~s~All", '~r~Server') then
				for i = 0, 255 do
					if not allPlayersIncludeSelf and i == PlayerId() then i = i + 1 end
					if NetworkIsPlayerActive(i) then
						local coords = GetEntityCoords(GetPlayerPed(i))
						AddExplosion(coords.x+2, coords.y+2, coords.z+2, 34, 100.0, true, false, 1.0)
                    end
                end               
			end
				if CKgangisontopmyfriend.Button("~y~Fire ~s~All", '~r~Server') then
				for i = 0, 256 do
					if not allPlayersIncludeSelf and i == PlayerId() then i = i + 1 end
					if NetworkIsPlayerActive(i) then
						local coords = GetEntityCoords(GetPlayerPed(i))
			...
			if CKgangisontopmyfriend.Button("~s~Remove ~s~All ~s~Players ~g~Weapon",'~r~Server') then
				local plist = GetActivePlayers()				  
				if not allPlayersIncludeSelf and i == PlayerId() then i = i + 1 end
				for i = 1, #plist do
					local id = plist[i]
			...
			if CKgangisontopmyfriend.ComboBox("~o~Spawn Prop ~s~All", ComboOptions.Props.Words, ComboOptions.Props.Selected, function(selectedIndex)
				if ComboOptions.Props.Selected ~= selectedIndex then
					ComboOptions.Props.Selected = selectedIndex
				end
			end) then 
				for i = 0, 255 do
					if not allPlayersIncludeSelf and i == PlayerId() then i = i + 1 end
					if NetworkIsPlayerActive(i) then
						TrollProp(GetPlayerPed(i), ComboOptions.Props.Values[ComboOptions.Props.Selected])
					end
                end
			end
		
			-- if
			-- 	CKgangisontopmyfriend.CheckBox(
			-- 	"~r~Explode All",
			-- 	blowall,
			-- 	function(enabled)
			-- 	blowall = enabled
			...
			-- end
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("LocalWepMenu") then
			
			if CKgangisontopmyfriend.MenuButton("Weapon Spawner", "LocalWepCategory") then
			end

			--if CKgangisontopmyfriend.MenuButton("Aimbot Options", "AimbotOptions") then
			--end

			if CKgangisontopmyfriend.CheckBox("Rapid Fire", Lux.Toggle.RapidFire) then
				Lux.Toggle.RapidFire = not Lux.Toggle.RapidFire
			end

			if CKgangisontopmyfriend.CheckBox("Silent Explosive Ammo", Lux.Toggle.SilentExplosiveAmmo) then
				Lux.Toggle.SilentExplosiveAmmo = not Lux.Toggle.SilentExplosiveAmmo
			end

			if CKgangisontopmyfriend.CheckBox("Explosive Ammo", Lux.Toggle.ExplosiveAmmo) then
				Lux.Toggle.ExplosiveAmmo = not Lux.Toggle.ExplosiveAmmo
			end
			if CKgangisontopmyfriend.CheckBox('Aimbot ~r~(!)', weaponsAimbotT) then
				weaponsAimbotT = not weaponsAimbotT
			end

			if CKgangisontopmyfriend.CheckBox('HVH ~r~(!)', HVH) then
				HVH = not HVH
			end

			-- SetWeaponDamageModifier(`WEAPON_PISTOL`, 100.0)
			if CKgangisontopmyfriend.Slider("Damage Modifier", SliderOptions.DamageModifier.Words, SliderOptions.DamageModifier.Selected, function(selectedIndex)
			...
			if CKgangisontopmyfriend.Button("~g~Give All Weapons", '~g~Native') then
			...
			if CKgangisontopmyfriend.Button("~r~Remove All Weapons", '~g~Native') then
				for i = 1, #t_Weapons do
					RemoveWeaponFromPed(PlayerPedId(), t_Weapons[i][1])
				end
			end

			if CKgangisontopmyfriend.Button("Set current weapon ammo", '~g~Native') then
			...
			if CKgangisontopmyfriend.CheckBox("Infinite Ammo", InfAmmo, function(enabled) InfAmmo = enabled SetPedInfiniteAmmoClip(PlayerPedId(), InfAmmo) end) then end	
		--	if CKgangisontopmyfriend.CheckBox("Vehicle Gun", VehicleGun, function(enabled) VehicleGun = enabled end) then end		
			if CKgangisontopmyfriend.CheckBox("Delete Gun", Lux.Toggle.DeleteGun) then 
				Lux.Toggle.DeleteGun = not Lux.Toggle.DeleteGun
			end

			CKgangisontopmyfriend.Display()
			-- [NOTE] Local Weapon Menu
		elseif CKgangisontopmyfriend.IsMenuOpened("LocalWepCategory") then
			CKgangisontopmyfriend.MenuButton("Melee", "LocalMeleeWeapons")
			CKgangisontopmyfriend.MenuButton("Handguns", "LocalSidearmWeapons")
			CKgangisontopmyfriend.MenuButton("Submachine Guns", "LocalSmgWeapons")
			CKgangisontopmyfriend.MenuButton("Shotguns", "LocalShotgunWeapons")
			CKgangisontopmyfriend.MenuButton("Assault Rifles", "LocalAssaultRifleWeapons")
			CKgangisontopmyfriend.MenuButton("Light Machine Guns", "LocalMachineGunWeapons")
			CKgangisontopmyfriend.MenuButton("Sniper Rifles", "LocalSniperRifles")
			CKgangisontopmyfriend.MenuButton("Heavy Weapons", "LocalHeavyWeapons")

			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("LocalMeleeWeapons") then
			local selectedWeapon = {}
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "melee" then
					if CKgangisontopmyfriend.Button(t_Weapons[i][2]) then
						PlaySoundFrontend(-1, "PICK_UP", "HUD_FRONTEND_DEFAULT_SOUNDSET", true)
			...
			-- CKgangisontopmyfriend.DrawWeaponPreview(selectedWeapon[menus[currentMenu].currentOption])
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("LocalSidearmWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "handguns" then
					if t_Weapons[i][6] then
						if weaponMkSelection[i] == nil then weaponMkSelection[i] = 1 end
						
						if CKgangisontopmyfriend.ComboBox(t_Weapons[i][2], ComboOptions.MK2.Words, weaponMkSelection[i], function(selectedIndex)
			...
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("LocalAssaultRifleWeapons") then
			for i = 1, #t_Weapons do
			...
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("LocalShotgunWeapons") then
			for i = 1, #t_Weapons do
			...
			CKgangisontopmyfriend.Display()	
		elseif CKgangisontopmyfriend.IsMenuOpened("LocalMachineGunWeapons") then
			for i = 1, #t_Weapons do
			...
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("LocalSmgWeapons") then
			for i = 1, #t_Weapons do
			...
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("LocalSniperRifles") then
			for i = 1, #t_Weapons do
			...
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("LocalHeavyWeapons") then
			for i = 1, #t_Weapons do
			...
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("LocalVehicleMenu") then

			if CKgangisontopmyfriend.MenuButton("Vehicle Spawner", "LocalVehicleSpawner") then
			end

			if CKgangisontopmyfriend.ComboBox("Vehicle Actions", ComboOptions.VehicleActions.Words, ComboOptions.VehicleActions.Selected, function(selectedIndex)
			...
			if CKgangisontopmyfriend.MenuButton("Modify Vehicle", "LSC") then end

			if CKgangisontopmyfriend.CheckBox("Easy Handling", Lux.Toggle.EasyHandling) then
				Lux.Toggle.EasyHandling = not Lux.Toggle.EasyHandling
			end

			if CKgangisontopmyfriend.CheckBox("Seatbelt (No Fall)", Lux.Toggle.VehicleNoFall) then
				Lux.Toggle.VehicleNoFall = not Lux.Toggle.VehicleNoFall
			end

			if CKgangisontopmyfriend.CheckBox("No Collision", Lux.Toggle.VehicleCollision, function(enabled) 
				Lux.Toggle.VehicleCollision = enabled
			end) then end

			if CKgangisontopmyfriend.ComboBox("Vehicle Hygiene", ComboOptions.DirtLevel.Words, ComboOptions.DirtLevel.Selected(Lux.Player.Vehicle), function(selectedIndex)
			...
			if CKgangisontopmyfriend.Button("Change License Plate", '~g~Native') then
				ChangeVehiclePlateText(Lux.Player.Vehicle)
			end

			if CKgangisontopmyfriend.CheckBox("Rainbow Vehicle Color", Lux.Toggle.RainbowVehicle, RainbowVehicle) then 
			end

			if CKgangisontopmyfriend.CheckBox("Vehicle Godmode", VehGod,
					function(enabled)
						VehGod = enabled
					end) 
				then
			end
			if CKgangisontopmyfriend.CheckBox("Speedboost ~g~SHIFT ~r~CTRL", VehSpeed,
					function(enabled)
					VehSpeed = enabled
					end)
				then
			...
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("LocalVehicleSpawner") then
			if CKgangisontopmyfriend.CheckBox("Spawn Inside", Lux.Toggle.SpawnInVehicle, function(enabled)
				Lux.Toggle.SpawnInVehicle = enabled
			end) then end
			
			if CKgangisontopmyfriend.CheckBox("Replace Current", Lux.Toggle.ReplaceVehicle, function(enabled) 
				Lux.Toggle.ReplaceVehicle = enabled 
			end) then end


			if CKgangisontopmyfriend.Button("Spawn Vehicle by Hash") then
				...
				elseif IsModelValid(modelName) and IsModelAVehicle(modelName) then
					...
					CKgangisontopmyfriend.SendNotification({text = string.format("%s is not a valid vehicle", modelName), type = 'error'})
				end
			end
			if CKgangisontopmyfriend.MenuButton("Compacts", "localcompacts") then end
			if CKgangisontopmyfriend.MenuButton("Sedans", "localsedans") then end
			if CKgangisontopmyfriend.MenuButton("SUVs", "localsuvs") then end
			if CKgangisontopmyfriend.MenuButton("Coupes", 'localcoupes') then end
			if CKgangisontopmyfriend.MenuButton("Muscle", 'localmuscle') then end
			if CKgangisontopmyfriend.MenuButton("Sports Classics", 'localsportsclassics') then end
			if CKgangisontopmyfriend.MenuButton("Sports", 'localsports') then end
			if CKgangisontopmyfriend.MenuButton("Super", 'localsuper') then end
			if CKgangisontopmyfriend.MenuButton('Motorcycles', 'localmotorcycles') then end
			if CKgangisontopmyfriend.MenuButton('Off-Road', 'localoffroad') then end
			if CKgangisontopmyfriend.MenuButton('Industrial', 'localindustrial') then end
			if CKgangisontopmyfriend.MenuButton('Utility', 'localutility') then end
			if CKgangisontopmyfriend.MenuButton('Vans', 'localvans') then end
			if CKgangisontopmyfriend.MenuButton('Cycles', 'localcycles') then end
			if CKgangisontopmyfriend.MenuButton('Boats', 'localboats') then end
			if CKgangisontopmyfriend.MenuButton('Helicopters', 'localhelicopters') then end
			if CKgangisontopmyfriend.MenuButton('Planes', 'localplanes') then end
			if CKgangisontopmyfriend.MenuButton('Service/Emergency/Military', 'localservice') then end
			if CKgangisontopmyfriend.MenuButton('Commercial/Trains', 'localcommercial') then end
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localcompacts') then
			for i = 1, #VehicleClass.compacts do
				...
				if CKgangisontopmyfriend.Button(vehname) then
					SpawnLocalVehicle(modelName, Lux.Toggle.ReplaceVehicle, Lux.Toggle.SpawnInVehicle)
				end
			end

			CKgangisontopmyfriend.DrawVehiclePreview('compacts')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localsedans') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('sedans')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localsuvs') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('suvs')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localcoupes') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('coupes')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localmuscle') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('muscle')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localsportsclassics') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('sportsclassics')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localsports') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('sports')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localsuper') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('super')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localmotorcycles') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('motorcycles')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localoffroad') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('offroad')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localindustrial') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('industrial')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localutility') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('utility')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localvans') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('vans')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localcycles') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('cycles')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localboats') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('boats')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localhelicopters') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('helicopters')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localplanes') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('planes')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localservice') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('service')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened('localcommercial') then
			...
			CKgangisontopmyfriend.DrawVehiclePreview('commercial')
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("LSC") then
			if Lux.Player.IsInVehicle then
				if CKgangisontopmyfriend.MenuButton("Bodywork", "lsc_bodywork") then
					LSC.UpdateMods()
				end
				if CKgangisontopmyfriend.MenuButton("Performance Tuning", "lsc_performance") then
					LSC.UpdateMods()
				end
			else
				if CKgangisontopmyfriend.Button("No vehicle found") then
				end
			end

			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("lsc_bodywork") then
				...
				for i, mod in pairs(LSC.vehicleMods) do
					...
					if modCount > 0 then
						if mod.meta == "modFrontWheels" or mod.meta == "modBackWheels" then
							if CKgangisontopmyfriend.ComboBox(mod.name, LSC.WheelType, installed['wheels'], function(selectedIndex, selectedItem)
								selectedIndex = selectedIndex - 1
								installed['wheels'] = selectedIndex
								SetVehicleWheelType(Lux.Player.Vehicle, selectedIndex)
								CKgangisontopmyfriend.SetSubTitle(mod.meta, selectedItem .. " Wheels")
							end, true) then
								if modCount > 0 then
							...
							if CKgangisontopmyfriend.MenuButton(mod.name, mod.meta) then end
						end
					end

					if mod.meta == "modXenon" then
						if CKgangisontopmyfriend.CheckBox(mod.name, installed['modXenon']) then
							ToggleVehicleMod(Lux.Player.Vehicle, 22, not installed['modXenon'])
							LSC.UpdateMods()
						end

						if installed['modXenon'] then
							if CKgangisontopmyfriend.ComboBox("Xenon Color", ComboOptions.XenonColor.Words, ComboOptions.XenonColor.Selected, function(selectedIndex)	
			...
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("lsc_performance") then
			local installed = currentMods
			if Lux.Player.IsInVehicle then
				SetVehicleModKit(Lux.Player.Vehicle, 0)
				for i, type in pairs(LSC.perfMods) do
					...
					if modCount > 0 then
						if CKgangisontopmyfriend.Slider(type.name, VehicleUpgradeWords[modCount], installed[type.meta], function(selectedIndex)
							selectedIndex = selectedIndex - 2
				...
				if CKgangisontopmyfriend.CheckBox("Turbo", installed['modTurbo'], function(enabled)
					installed['modTurbo'] = enabled
					ToggleVehicleMod(Lux.Player.Vehicle, 18, enabled)
				end) then end
				
				if CKgangisontopmyfriend.ComboBox("Engine Power", ComboOptions.EnginePower.Words, ComboOptions.EnginePower.Selected, function(selectedIndex)
			...
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("ServerMenu") then
			CKgangisontopmyfriend.SetSubTitle("ServerMenu", "Server Options - " .. GetCurrentServerEndpoint())
			if CKgangisontopmyfriend.MenuButton("Resource List", "ServerResources") then end
			if CKgangisontopmyfriend.MenuButton("ESX Boss Options", "ESXBoss") then end
			if CKgangisontopmyfriend.MenuButton("Money Options", "ServerMoneyOptions") then end
			if CKgangisontopmyfriend.MenuButton("ESX Misc Options", "ESXMisc") then end
			if CKgangisontopmyfriend.MenuButton("ESX Drug Options", "ESXDrugs") then end
			if CKgangisontopmyfriend.MenuButton("VRP Options", "VRPOptions") then end
			if CKgangisontopmyfriend.MenuButton("Misc Options", "MiscServerOptions") then end

			CKgangisontopmyfriend.Display()
			
		elseif CKgangisontopmyfriend.IsMenuOpened("MenuSettings") then
			if CKgangisontopmyfriend.MenuButton("Change Color Theme", "MenuSettingsColor") then
			end
			if CKgangisontopmyfriend.MenuButton("Credits", "MenuSettingsCredits") then
			end
			if CKgangisontopmyfriend.Button("~r~Kill Menu") then
				isMenuEnabled = false
			end
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("MenuSettingsColor") then
			if CKgangisontopmyfriend.CheckBox("Dynamic Theme", dynamicColorTheme, function(enabled) dynamicColorTheme = enabled end) then
			end
			if CKgangisontopmyfriend.Button("Red", nil, themeColors.red) then
				_menuColor.base = themeColors.red
			end
			if CKgangisontopmyfriend.Button("Orange", nil, themeColors.orange) then
				_menuColor.base = themeColors.orange
			end
			if CKgangisontopmyfriend.Button("Yellow", nil, themeColors.yellow) then
				_menuColor.base = themeColors.yellow
			end
			if CKgangisontopmyfriend.Button("Green", nil, themeColors.green) then
				_menuColor.base = themeColors.green
			end
			if CKgangisontopmyfriend.Button("Blue", nil, themeColors.blue) then
				_menuColor.base = themeColors.blue
			end
			if CKgangisontopmyfriend.Button("Purple", nil, themeColors.purple) then
				_menuColor.base = themeColors.purple
			end

			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("MenuSettingsCredits") then
			for _, v in pairs(contributors) do 
				if CKgangisontopmyfriend.Button(v[1], v[2]) then end 
			end
			
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("ServerResources") then
			--for _, resource in pairs(validResources) do
			for i = 0, #Get_Res do
				local resource = Get_Res[i]
				if CKgangisontopmyfriend.MenuButton(resource, 'ResourceData') then
					SelectedResource = resource
				end
			end
			CKgangisontopmyfriend.Display()
		
		elseif CKgangisontopmyfriend.IsMenuOpened('ResourceData') then
			CKgangisontopmyfriend.SetSubTitle('ResourceData', SelectedResource .. " > Data")
			local LOAD_data_script = LoadResourceFile(SelectedResource, "client/main.lua")
			local script_data = false
			if LOAD_data_script then
				if string.sub(LOAD_data_script, 1, string.len("Trigger")) == "Trigger" then
					print(LOAD_data_script)
				end
			end
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("ESXBoss") then

			if CKgangisontopmyfriend.Button("~c~Mechanic") then
				TriggerEvent("esx_society:openBossMenu", "mecano", function(data, menu) setMenuVisible(currentMenu, false) end)
				
			elseif CKgangisontopmyfriend.Button("~b~Police") then
				TriggerEvent("esx_society:openBossMenu","police",function(data, menu) setMenuVisible(currentMenu, false) end)
			elseif CKgangisontopmyfriend.Button("~r~Ambulance") then
				TriggerEvent("esx_society:openBossMenu","ambulance",function(data, menu) setMenuVisible(currentMenu, false) end)
			elseif CKgangisontopmyfriend.Button("~y~Taxi") then
				TriggerEvent("esx_society:openBossMenu","taxi",function(data, menu) setMenuVisible(currentMenu, false) end)
			elseif CKgangisontopmyfriend.Button("~g~Real Estate") then
				TriggerEvent("esx_society:openBossMenu","realestateagent",function(data, menu) setMenuVisible(currentMenu, false) end)
			elseif CKgangisontopmyfriend.Button("~p~Gang") then
				TriggerEvent("esx_society:openBossMenu","gang",function(data, menu) setMenuVisible(currentMenu, false) end)
			elseif CKgangisontopmyfriend.Button("~o~Car Dealer") then
				TriggerEvent("esx_society:openBossMenu","cardealer",function(data, menu) setMenuVisible(currentMenu, false) end)
			elseif CKgangisontopmyfriend.Button("~y~Banker") then
				TriggerEvent("esx_society:openBossMenu","banker",function(data, menu) setMenuVisible(currentMenu, false) end)  
			end

			CKgangisontopmyfriend.Display()
		
		elseif CKgangisontopmyfriend.IsMenuOpened("ServerMoneyOptions") then

			if CKgangisontopmyfriend.Button("~g~ESX ~s~Caution Give Back") then
			...
			if CKgangisontopmyfriend.Button("~g~ESX ~s~TruckerJob Pay") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent('esx_truckerjob:pay', result)
				end
			end
			if CKgangisontopmyfriend.Button("~g~ESX ~s~Admin Give Bank") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveBank', result)
				end
			end
			if CKgangisontopmyfriend.Button("~g~ESX ~s~Admin Give Cash") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveCash', result)
				end
			end
			if CKgangisontopmyfriend.Button("~g~ESX ~s~GOPostalJob Pay") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent("esx_gopostaljob:pay", result)
				end
			end
			if CKgangisontopmyfriend.Button("~g~ESX ~s~BankerJob Pay") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent("esx_banksecurity:pay", result)
				end
			end
			if CKgangisontopmyfriend.Button("~g~ESX ~s~Slot Machine") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent("esx_slotmachine:sv:2", result)
				end
			end
			if CKgangisontopmyfriend.Button("~g~ESX ~s~WindowCleaner") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
		...
		elseif CKgangisontopmyfriend.IsMenuOpened("ESXMisc") then

			if CKgangisontopmyfriend.Button("~w~Set hunger to ~g~100%", '~g~Native') then
				TriggerEvent("esx_status:set", "hunger", 1000000)
			elseif CKgangisontopmyfriend.Button("~w~Set thirst to ~g~100%", '~g~Native') then
				TriggerEvent("esx_status:set", "thirst", 1000000)
			elseif CKgangisontopmyfriend.Button("~g~ESX ~r~Revive ~s~ID", '~r~Server') then
			...
			elseif CKgangisontopmyfriend.Button("Send All Bill", '~r~Server') then
			...
			elseif CKgangisontopmyfriend.Button("~b~Handcuff ~s~ID", '~r~Server') then
				local id = KeyboardInput("Enter Player ID", "", 3)
				if id then
					TriggerServerEvent('esx_policejob:handcuff', GetPlayerServerId(id))
				end
			elseif CKgangisontopmyfriend.Button("~g~ESX ~s~Register Identity", '~y~Client') then
				CKgangisontopmyfriend.CloseMenu()
				TriggerEvent('esx_identity:showRegisterIdentity')   
			elseif CKgangisontopmyfriend.Button("~g~ESX ~w~Get all licenses", '~y~Client') then
			...
			elseif CKgangisontopmyfriend.Button('~g~ESX ~s~Skin Changer', '~y~Client') then
				CKgangisontopmyfriend.CloseMenu()
				TriggerCustomEvent(false, 'esx_skin:openRestrictedMenu', function(data, menu) end)
			elseif CKgangisontopmyfriend.Button("~g~ESX ~s~Unrestrain Handcuffs", "~y~Client") then
				TriggerCustomEvent(false, "esx_policejob:unrestrain")
			end
			CKgangisontopmyfriend.Display()
		
		elseif CKgangisontopmyfriend.IsMenuOpened("MiscServerOptions") then

			if CKgangisontopmyfriend.Button("Send Discord Message") then
				local Message = KeyboardInput("Enter message to send", "", 100)
				TriggerServerEvent("DiscordBot:plaLittleACyerDied", Message, "1337")
				CKgangisontopmyfriend.SendNotification({text = "Sent message:~n~" .. Message .. "", type = "success"})
			elseif CKgangisontopmyfriend.Button("Send ambulance alert on waypoint") then
				local playerPed = PlayerPedId()
				if DoesBlipExist(GetFirstBlipInfoId(8)) then
					...
					CKgangisontopmyfriend.SendNotification("~g~Ambulance alert sent to waypoint!")
				else
					CKgangisontopmyfriend.SendNotification("~r~No waypoint set!")
				end

			elseif CKgangisontopmyfriend.Button("Spoof text message (GCPHONE)") then
				...
				if transmitter then
					if receiver then
						if message then
							...
							CKgangisontopmyfriend.SendNotification("~r~You must specify a message.")
						end
					else
						CKgangisontopmyfriend.SendNotification("~r~You must specify a receiving number.")
					end
				else
					CKgangisontopmyfriend.SendNotification("~r~You must specify a transmitting number.")
				end
			elseif CKgangisontopmyfriend.Button("Spoof Chat Message") then
			...
			elseif CKgangisontopmyfriend.Button("esx_mugging Give item") then
				local itemName = KeyboardInput("Enter item name", "", 20)
				if itemName then
					TriggerServerEvent('esx_mugging:giveItems', (itemName))
					CKgangisontopmyfriend.SendNotification("Successfully given item ~g~" .. itemName)
				else
					CKgangisontopmyfriend.SendNotification("~r~You must specify an item")
				end
			end

			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("OnlinePlayersMenu") then
			...
			for i = 1, #plist do
				...
				if CKgangisontopmyfriend.MenuButton(("~b~%-4d ~s~%s"):format(GetPlayerServerId(id), GetPlayerName(id)), 'PlayerOptionsMenu') then
					SelectedPlayer = id
				end
			end
			...
			CKgangisontopmyfriend.DrawPlayerInfo(onlinePlayerSelected[index])
			CKgangisontopmyfriend.Display()
		
		elseif CKgangisontopmyfriend.IsMenuOpened("PlayerOptionsMenu") then
			CKgangisontopmyfriend.SetSubTitle("PlayerOptionsMenu", "Player Options [" .. GetPlayerName(SelectedPlayer) .. "]")
			
			if CKgangisontopmyfriend.Button("Spectate", (Lux.Player.Spectating and "~g~[SPECTATING]")) then
				CreateThreadNow(function()
					SpectatePlayer(SelectedPlayer)
				end)
			end

			if CKgangisontopmyfriend.Button("Teleport To Player") then
				Lux.Game:TeleportToPlayer(SelectedPlayer)
			end

			if CKgangisontopmyfriend.Button("Teleport into Vehicle") then
				TeleportToPlayerVehicle(SelectedPlayer)
			end

			if CKgangisontopmyfriend.MenuButton("Weapon Options", "OnlineWepMenu") then end
			if CKgangisontopmyfriend.MenuButton("Vehicle Options", "OnlineVehicleMenuPlayer") then end
			if CKgangisontopmyfriend.MenuButton("ESX Options", "ESXMenuPlayer") then end
			if CKgangisontopmyfriend.MenuButton("Troll Options", "OnlineTrollMenu") then end
			if CKgangisontopmyfriend.Button("~r~Silent Explode") then
				AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 2, 100000.0, false, true, 0)
			end
			if CKgangisontopmyfriend.Button("~y~Explode") then
				AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 2, 100000.0, true, false, 100000.0)
			end
			if CKgangisontopmyfriend.Button("Give All Weapons", '~g~Native') then
				for i = 1, #t_Weapons do
					GiveWeaponToPed(GetPlayerPed(SelectedPlayer), t_Weapons[i][1], 128, false, false)
				end
			end
			if CKgangisontopmyfriend.Button("Remove All Weapons", '~g~Native') then
			...
			if CKgangisontopmyfriend.Button("~o~Clone ~s~Outfit", '~g~Native') then
				clonePedOutfit(PlayerPedId(), GetPlayerPed(SelectedPlayer))
			end

			CKgangisontopmyfriend.DrawPlayerInfo(SelectedPlayer)
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("OnlineTrollMenu") then
			
			if CKgangisontopmyfriend.Button("Airstrike") then
				AirstrikePlayer(SelectedPlayer)
			end

			if CKgangisontopmyfriend.Button("Taze Player") then
				TazePlayer(SelectedPlayer)
			end

			CKgangisontopmyfriend.DrawPlayerInfo(SelectedPlayer)
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("ESXMenuPlayer") then
			if CKgangisontopmyfriend.Button("~g~ESX ~s~Send Bill") then
			...
			elseif CKgangisontopmyfriend.Button("~g~ESX ~s~Handcuff Player") then
				TriggerServerEvent('esx_policejob:handcuff', GetPlayerServerId(SelectedPlayer))
			elseif CKgangisontopmyfriend.Button("~g~ESX ~s~Revive player") then
				TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(SelectedPlayer))
			elseif CKgangisontopmyfriend.Button("~g~ESX ~s~Unjail player") then
			...
			elseif CKgangisontopmyfriend.Button("~g~ESX ~s~Open ~s~inventory") then
		...
		elseif CKgangisontopmyfriend.IsMenuOpened("OnlineWepMenu") then
			CKgangisontopmyfriend.SetSubTitle("OnlineWepMenu", "Weapon Options - " .. GetPlayerName(SelectedPlayer) .. "")
			CKgangisontopmyfriend.MenuButton("Give Weapon", "OnlineWepCategory")

			CKgangisontopmyfriend.DrawPlayerInfo(SelectedPlayer)
			CKgangisontopmyfriend.Display()
		
		elseif CKgangisontopmyfriend.IsMenuOpened("OnlineWepCategory") then
			CKgangisontopmyfriend.SetSubTitle("OnlineWepCategory", "Give Weapon - " .. GetPlayerName(SelectedPlayer) .. "")

			CKgangisontopmyfriend.MenuButton("Melee", "OnlineMeleeWeapons")
			CKgangisontopmyfriend.MenuButton("Handguns", "OnlineSidearmWeapons")
			CKgangisontopmyfriend.MenuButton("Submachine Guns")
			CKgangisontopmyfriend.MenuButton("Shotguns", "OnlineShotgunWeapons")
			CKgangisontopmyfriend.MenuButton("Assault Rifles", "OnlineAutorifleWeapons")
			CKgangisontopmyfriend.MenuButton("Light Machine Guns")
			CKgangisontopmyfriend.MenuButton("Sniper Rifles")
			CKgangisontopmyfriend.MenuButton("Heavy Weapons")

			CKgangisontopmyfriend.DrawPlayerInfo(SelectedPlayer)
			CKgangisontopmyfriend.Display()
		
		elseif CKgangisontopmyfriend.IsMenuOpened("OnlineMeleeWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "melee" then
					if CKgangisontopmyfriend.Button(t_Weapons[i][2]) then
		...
		elseif CKgangisontopmyfriend.IsMenuOpened("OnlineSidearmWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "handguns" then
					if CKgangisontopmyfriend.Button(t_Weapons[i][2]) then
		...
		elseif CKgangisontopmyfriend.IsMenuOpened("OnlineAutorifleWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "assaultrifles" then
					if CKgangisontopmyfriend.Button(t_Weapons[i][2]) then
		...
		elseif CKgangisontopmyfriend.IsMenuOpened("OnlineShotgunWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "shotguns" then
					if CKgangisontopmyfriend.Button(t_Weapons[i][2]) then
		...
		elseif CKgangisontopmyfriend.IsMenuOpened("OnlineVehicleMenuPlayer") then
			CKgangisontopmyfriend.SetSubTitle("OnlineVehicleMenuPlayer", "Vehicle Options [" .. GetPlayerName(SelectedPlayer) .. "]")
			if CKgangisontopmyfriend.Button("Spawn Vehicle") then
				local ped = GetPlayerPed(SelectedPlayer)
				...
				if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
					...
					CKgangisontopmyfriend.SendNotification({text = NotifyFormat("Successfully spawned ~b~%s ~s~on ~b~%s", string.lower(GetDisplayNameFromVehicleModel(ModelName)), GetPlayerName(SelectedPlayer)), type = "info"})
				else
					CKgangisontopmyfriend.SendNotification({text = "Model is not valid", type = "error"})
				end
			end
			if CKgangisontopmyfriend.Button("Spawn Owned Vehicle") then
				local ped = GetPlayerPed(SelectedPlayer)
				local ModelName = KeyboardInput("Enter Vehicle Spawn Name", "", 20)
				...
				if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
					...
					CKgangisontopmyfriend.SendNotification({ text = "Vehicle model " .. ModelName .. " does not exist!", type = "error"})
				end
			end
			if CKgangisontopmyfriend.Button("Kick From Vehicle") then
				ClearPedTasksImmediately(GetPlayerPed(SelectedPlayer))
			end
			if CKgangisontopmyfriend.Button("Delete Vehicle") then
				local playerPed = GetPlayerPed(SelectedPlayer)
				local veh = GetVehiclePedIsIn(playerPed)
				RemoveVehicle(veh)
			end
			if CKgangisontopmyfriend.Button("Destroy Engine") then
			...
			if CKgangisontopmyfriend.Button("Steal Vehicle") then
			...
			if CKgangisontopmyfriend.Button("Repair Vehicle") then
				local ped = GetPlayerPed(SelectedPlayer)
				local vehicle = GetVehiclePedIsUsing(ped)
				RepairVehicle(vehicle)
			end

			if CKgangisontopmyfriend.Button("Vandalize Car") then
				local playerPed = GetPlayerPed(SelectedPlayer)
				...
				CKgangisontopmyfriend.SendNotification("~g~Vehicle Fucked Up!")
			end
		...
		for i, mods in pairs(LSC.vehicleMods) do
			if mods.meta == "modHorns" then
				if CKgangisontopmyfriend.IsMenuOpened(mods.meta) then
					for j = 0, 51, 1 do
						if j == currentMods[mods.meta] then
							if CKgangisontopmyfriend.Button(LSC.GetHornName(j), "Installed", nil, _menuColor.base) then 
								RemoveVehicleMod(Lux.Player.Vehicle, mods.id)
								LSC.UpdateMods()
							end
						else
							if CKgangisontopmyfriend.Button(LSC.GetHornName(j), "Not Installed") then 
			...
			elseif mods.meta == "modFrontWheels" or mods.meta == "modBackWheels" then
				if CKgangisontopmyfriend.IsMenuOpened(mods.meta) then
					...
					for j = 0, modCount, 1 do
						...
						if modName then
							if j == currentMods[mods.meta] then
								if CKgangisontopmyfriend.Button(GetLabelText(modName), "Installed", nil, _menuColor.base) then 
									RemoveVehicleMod(Lux.Player.Vehicle, mods.id)
									LSC.UpdateMods()
								end
							else
								if CKgangisontopmyfriend.Button(GetLabelText(modName), "Not Installed") then 
									SetVehicleMod(Lux.Player.Vehicle, mods.id, j)
									LSC.UpdateMods()
								end
							end
...
function GateKeep()
	...
	if _gatekeeper then
		if name then
			...
			_auth = true
			CKgangisontopmyfriend.SendNotification({ text = "~r~ERROR: ~w~Nejsi opravnen k pouziti ~h~CK MENU", type = "error"})
		end
	else
		_auth = true
		...
		if input == _secretKey then
			_gatekeeper = true
			CKgangisontopmyfriend.SendNotification({ text = "~g~SUCCESS: ~w~Keycode validated.", type = "success"})
		else
			_auth = true
			...
			_notifMsg2 = ""
			_errorCode = 1
			CKgangisontopmyfriend.SendNotification({ text = "~r~ERROR: ~w~Invalid keycode", type = "error"})
		end
	end
end
